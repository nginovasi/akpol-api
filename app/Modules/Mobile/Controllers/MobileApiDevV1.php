...
class MobileApiDevV1 extends BaseController
{
    private $mobileModel;
	...
	public function scheduleDetailLecturer(){
		...
		$query = $this->db->query("SELECT a.id as id_jadwal,f.namagadik,f.photopath,c.kelompok,g.id as id_mata_pelajaran,b.id_batalyon,g.`mata_pelajaran`,e.`judul`,e.`deskripsi`,e.pertemuan_ke,a.tanggal,a.is_absensi_pendidik,a.`jam_mulai`,a.`jam_selesai`,d.`nama` as lokasi_kelas,
...
GROUP BY a.pertemuan_ke");
		...
		// 							if(m.id_jadwal is not null, 1,0) as is_label
		// 							from t_bahan_ajar a
		// 							LEFT JOIN (SELECT * from t_jadwal where id_kelompok_taruna='".$id_kelompok_taruna."') b on a.id=b.id_bahan_ajar 
		// 							left join m_kelompok c
		// 								on b.id_kelompok_taruna = c.id
		// 							left join m_user_taruna o
		// 								on b.id=o.id_kelompok
		// 							left join m_sm_batalyon d
		// 								on d.id = o.id_batalyon
		// 							left join m_mata_pelajaran e
		// 								on e.id = a.id_mata_pelajaran
		...
		// 												LEFT JOIN t_jadwal b on a.id=b.id_bahan_ajar
		// 												left join m_user_taruna o on b.id_kelompok_taruna=o.id_kelompok
		// 										where a.id_mata_pelajaran='".$id_mata_pelajaran."' and a.is_ujian=0 and a.id_user_pendidik='".$id_pengajar."' and b.is_absensi_pendidik = 0 and (o.id_kelompok='".$id_kelompok_taruna."' or o.id_kelompok is null)  order by a.pertemuan_ke asc limit 1) m  on m.id_jadwal = b.id
		// 							LEFT JOIN t_program_studi_mata_pelajaran n on n.id_mata_pelajaran=e.id
		...
		// 							and (o.id_kelompok='".$id_kelompok_taruna."' or o.id_kelompok is null) 
		// 							and a.is_ujian=0 
		// 							");
	...
	public function sessionClassStudent(){
		...
		// from t_bahan_ajar a 
		// 	-- LEFT JOIN t_jadwal b on a.id=b.id_bahan_ajar
		// 	LEFT JOIN (SELECT * from t_jadwal where id_kelompok_taruna='".$id_kelompok."') b on a.id=b.id_bahan_ajar 
		...
		// 			LEFT JOIN m_user_taruna d on a.id_semester=d.id_semester and d.id_batalyon=a.id_batalyon
		// 			where a.id_mata_pelajaran='".$id_mata_pelajaran."' and d.id_m_user='".$id_user."'  and b.is_absensi_pendidik = 0 order by a.pertemuan_ke asc limit 1) g
		// 			on g.id_jadwal = b.id
		// 	LEFT JOIN m_user_pendidik h on b.id_user_pendidik=h.id_m_user
		...
		// 	where a.id_mata_pelajaran='".$id_mata_pelajaran."' and i.id_m_user='".$id_user."'  and a.is_ujian = 0 order by a.id asc");
